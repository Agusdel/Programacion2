...
int Oferta(DealOrNoDeal deal){ /// Promedia los valores disponibles (incluyendo el maletin elegido) y multiplica por constante<1 para reducirlo.
    ...
    oferta = oferta*(CANTIDAD_MALETINES-cantMaletinesDisponibles)/(CANTIDAD_MALETINES+1-cantMaletinesDisponibles); // Cuantos menos maletines quedan, es más alta la proporción del promedio que se ofrece.
    return oferta;
}
